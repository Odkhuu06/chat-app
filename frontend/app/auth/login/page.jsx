...
export default function LoginPage() {
  ...
  const [loading, setLoading] = useState(false); // submit үед button disable
  const [error, setError] = useState(""); // алдааны state
  ...
  const submit = async (e) => {
    e.preventDefault();
    setLoading(true);
    setError(""); // хуучин алдааг арилгах

    try {
      const res = await API.post("/api/auth/login", { username, password });
      saveToken(res.data.token);
      router.push("/chat"); // client-side navigation
    } catch (err) {
